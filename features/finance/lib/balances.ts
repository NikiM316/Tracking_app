 ...
 * debits `account_id` while crediting `transfer_account_id` on the same row.
 * A transfer with no destination is skipped rather than counted one-sided,
 * which would silently unbalance the books.
 */
